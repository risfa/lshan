...
$sql=$dbo->prepare("insert into poll_answer(IdCustomer,qst_id,opt) values('$_SESSION[id_customer]',$qst_id,:opt)");
...
$main= array("data"=>array("q1"=>"$row->qst","opt1"=>"$row->opt1","opt2"=>"$row->opt2","opt3"=>"$row->opt3","opt4"=>"$row->opt4","opt5"=>"$row->opt5","qst_id"=>"$qst_id"),"next"=>"$next");
