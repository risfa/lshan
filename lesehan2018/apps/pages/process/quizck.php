...
$sql=$dbo->prepare("insert into quiz_answer(IdCustomer,qst_id,opt) values('$_SESSION[id_customer]',$qst_id,:opt)");
...
$qst_id=$qst_id+1;
$no_questions = $dbo->query("select count(qst_id) from quiz_qst")->fetchColumn();
if($qst_id > $no_questions){
$next='F'; // Flag is set to display thank you message
}else{
$next='T'; // Flag is set to display next question

$count=$dbo->prepare("select * from quiz_qst where qst_id=$qst_id");
if($count->execute()){
