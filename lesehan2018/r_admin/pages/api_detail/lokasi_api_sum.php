...
date_default_timezone_set('Asia/Jakarta'); 
    ...
    while($loop = mysqli_fetch_array($lok)){
        ...
        $output.='<div class="col-md-6"><div class="widget widget-table action-table">
                    <h3 style="text-transform:UPPERCASE;" id="open_tab">'.$loop['Lokasi'].'</h3>';
                   $output.='<div class="widget-content">
                        <table class="table table-striped table-bordered" style="margin-top:-25px;">
                            ...
                            <tbody>
                            <br>';
            $jumlah =mysqli_query($connect,"SELECT * FROM tb_traffic_kendaraan WHERE IdLokasi = $loop[0] ");
    ...
    $output .= "<script> $('#open_tab').click(function(){
                            window.open('http://joker.5dapps.com/pertamina/lesehan2018/r_admin/index.php?menu=laporan_foto');
                    }) 
                </script>";
